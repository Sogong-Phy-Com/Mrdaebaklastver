...
#### 1. 주문 하기
- **주요 흐름**: 주문 정보 입력 → 재고 확인 → 결제 처리 → 주문 저장
- **참여 컴포넌트**: React Client, OrderController, OrderService, InventoryService, PaymentService
- **데이터베이스**: Order DB, Inventory DB

#### 2. 주문 내역 조회
- **주요 흐름**: 조회 요청 → 인증 확인 → 주문 조회 → 목록 반환
- **참여 컴포넌트**: React Client, OrderController, OrderService
- **데이터베이스**: Order DB

#### 3. 예약 변경 요청
- **주요 흐름**: 변경 요청 → 수수료 계산 → 차액 결제(필요시) → 요청 저장
- **참여 컴포넌트**: React Client, ReservationChangeRequestController, OrderChangeRequestService, PaymentService
- **데이터베이스**: Order DB, ChangeRequest DB

### 관리자 시퀀스 다이어그램 (3개)

#### 1. 주문 관리 (승인/반려)
- **주요 흐름**: 주문 목록 조회 → 주문 승인/반려 → 상태 업데이트
- **참여 컴포넌트**: React Client, AdminController, OrderService
- **데이터베이스**: Order DB

#### 2. 예약 변경 요청 관리
- **주요 흐름**: 변경 요청 목록 조회 → 승인/반려 → 주문 업데이트 → 환불 처리(필요시)
- **참여 컴포넌트**: React Client, AdminReservationChangeRequestController, OrderChangeRequestService, PaymentService
- **데이터베이스**: ChangeRequest DB, Order DB

#### 3. 계정 관리
- **주요 흐름**: 계정 목록 조회 → 계정 승인/반려 → 상태 업데이트
- **참여 컴포넌트**: React Client, AdminController, AuthService
- **데이터베이스**: User DB

### 직원 시퀀스 다이어그램 (3개)

#### 1. 재고 확인
- **주요 흐름**: 재고 조회 요청 → 권한 확인 → 재고 정보 조회 → 정보 반환
- **참여 컴포넌트**: React Client, InventoryController, InventoryService
- **데이터베이스**: Inventory DB

#### 2. 스케줄 조회
- **주요 흐름**: 스케줄 조회 요청 → 권한 확인 → 스케줄 조회 → 정보 반환
- **참여 컴포넌트**: React Client, EmployeeController, DeliverySchedulingService
- **데이터베이스**: Schedule DB

#### 3. 주문 관리 (상태 변경)
- **주요 흐름**: 주문 목록 조회 → 조리 시작 → 재고 차감 → 상태 업데이트
