...
### 프레젠테이션 계층
- **React Client**: 메인 클라이언트 애플리케이션
- **OrderPage**: 주문 페이지
- **OrdersPage**: 주문 내역 페이지
- **AdminPages**: 관리자 페이지들
- **EmployeePages**: 직원 페이지들
- **VoiceOrderPage**: 음성 주문 페이지
- **Web Speech API**: 브라우저 음성 인식 API

### 애플리케이션 계층 - Controller
- **OrderController**: 주문 관련 REST API
- **AdminController**: 관리자 기능 REST API
- **EmployeeController**: 직원 기능 REST API
- **AuthController**: 인증 REST API
- **InventoryController**: 재고 관리 REST API
- **VoiceOrderController**: 음성 주문 REST API
...
- **AuthService**: 인증 로직
- **DeliverySchedulingService**: 배달 스케줄 관리
- **OrderChangeRequestService**: 예약 변경 요청 처리
- **VoiceOrderSessionService**: 음성 주문 세션 관리
- **JwtService**: JWT 토큰 처리

### 애플리케이션 계층 - Repository
- **OrderRepository**: 주문 데이터 접근
- **OrderItemRepository**: 주문 항목 데이터 접근
- **InventoryRepository**: 재고 데이터 접근
- **UserRepository**: 사용자 데이터 접근
- **MenuItemRepository**: 메뉴 데이터 접근
- **ScheduleRepository**: 스케줄 데이터 접근

### 데이터 계층
- **메인 DB (mrdabak.db)**: 사용자, 메뉴 정보
- **재고 DB (inventory.db)**: 재고 정보
- **주문 DB (orders.db)**: 주문 정보
- **스케줄 DB (schedule.db)**: 배달 스케줄 정보
